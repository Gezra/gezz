import{PolymerElement,html,Debouncer,timeOut,getGooglePayConfig}from"./shop-app.js";const $_documentContainer=document.createElement("template");$_documentContainer.innerHTML=`<dom-module id="shop-checkbox">
  <template>
    <style>
  ...
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$1.content);const googlepayConfig=getGooglePayConfig();class ShopCheckout extends PolymerElement{static get template(){return html`
    ...
    `}static get is(){return"shop-checkout"}static get properties(){return{/**
       ...
       */cart:Array,googlepayConfig:{type:Object,value:()=>googlepayConfig},/**
       ...
       * True when waiting for the server to repond.
       */_hasItems:{type:Boolean,computed:"_computeHasItem(cart.length)"}}}static get observers(){return["_updateState(routeActive, routeData)","_refreshDetails(cart, total)"]}_submit(e){if(this._validateForm()){// To send the form data to the server:
...
}}constructor(){super();this._onGooglePayPaymentDataResult=this._onGooglePayPaymentDataResult.bind(this);this._onGooglePayPaymentDataChanged=this._onGooglePayPaymentDataChanged.bind(this)}/**
...
body.cartItemsId=[];body.cartItemsQuantity=[];this.cart.forEach(cartItem=>{body.cartItemsId.push(cartItem.item.name);body.cartItemsQuantity.push(cartItem.quantity)})}/**
     ...
     */_didReceiveResponse(e){let response=e.detail.response;this.response=response;this._setWaiting(!0);if(response.success){this._reset();this.dispatchEvent(new CustomEvent("clear-cart",{bubbles:!0,composed:!0}));this.dispatchEvent(new CustomEvent("confirmation",{bubbles:!0,composed:!0}))}else{this._pushState("error")}}_toggleBillingAddress(e){this.hasBillingAddress=e.target.checked;if(this.hasBillingAddress){this.$.billAddress.focus()}}_computeHasItem(cartLength){return 0<cartLength}_formatPrice(total){return isNaN(total)?"":"$"+total.toFixed(2)}_getEntryTotal(entry){return this._formatPrice(entry.quantity*entry.item.price)}_visibleChanged(visible){if(!visible){return}// Reset the UI states
this._reset();// Notify the page's title
this.dispatchEvent(new CustomEvent("change-section",{bubbles:!0,composed:!0,detail:{title:"Checkout"}}))}_onGooglePayPaymentDataResult(paymentResponse){this.googlepayConfig.onLoadPaymentData.bind(this)(paymentResponse,{items:this.cart,type:"cart",method:"google-pay"})}_getGooglePayTransactionInfo(shippingOption){if(this.cart){const paymentRequest=this.googlepayConfig.buildPaymentRequest(this.cart.map(i=>({label:`${i.item.title} x ${i.quantity}`,type:"LINE_ITEM",price:(i.item.price*i.quantity).toFixed(2)})),shippingOption);return paymentRequest.transactionInfo}return null}_onGooglePayPaymentDataChanged(paymentData){if(paymentData.shippingOptionData.id){const shippingOption=this.googlepayConfig.shippingOptions.find(option=>option.id===paymentData.shippingOptionData.id);if(shippingOption){const transactionInfo=this._getGooglePayTransactionInfo(shippingOption);return{newTransactionInfo:transactionInfo}}}return{}}_refreshDetails(){this.$.googlePayButton.paymentRequest.transactionInfo=this._getGooglePayTransactionInfo()}}customElements.define(ShopCheckout.is,ShopCheckout);